...
let flag1 = 1, flag2 = 1, flag3 = 1, flag4 = 1, flag5 = 1, flag6 = 1, flag7 = 1, flag8 = 1;
...
function checkPawnStrike(x, y){
	if(squares[`${x}${y - 1}`]?.querySelector('img'))
		if(squares[`${x}${y - 1}`]?.querySelector('img').getAttribute('value').slice(0, 5) != index) strike(x, y - 1);
	if(squares[`${x}${y + 1}`]?.querySelector('img'))
		if(squares[`${x}${y + 1}`]?.querySelector('img').getAttribute('value').slice(0, 5) != index) strike(x, y + 1);
}
...
function highlightTileRook(row, column) {
	for (let i = column; i <= 7; i++) {
		...
		if (squares[`${z}${i}`]?.querySelector('img') != -1 && flag1 == 1) {
			glowTile(z, i);
	...
	for (let j = column; j >= 0; j--) {
		let z = String.fromCharCode(row);
		if (squares[`${z}${j}`]?.querySelector("img") != -1 && flag2 == 1) {
			glowTile(z, j);
	...
	for (let a = row; a <= 104; a++) {
		...
		if (squares[`${p}${column}`]?.querySelector("img") != -1 && flag3 == 1) {
			glowTile(p, column);
	...
	for (let b = row; b >= 97; b--) {
		...
		if (squares[`${r}${column}`]?.querySelector("img") != -1 && flag4 == 1) {
			glowTile(r, column);
...
function highlightTileBishop(row, column) {
	for( let x = 0; x <= 7; x++) {
		...
		if(squares[`${v}${column - x}`]?.querySelector('img') != -1 && flag5 == 1) {
			glowTile(v, column - x)
		...
		if(squares[`${w}${column - x}`]?.querySelector('img') != -1 && flag6 == 1) {
			glowTile(w, column - x)
		...
		if(squares[`${v}${column + x}`]?.querySelector('img') != -1 && flag7 == 1) {
			glowTile(v, column + x)
			if(squares[`${vNext}${column + x + 1}`]?.querySelector('img')) {
				flag7 = 0
				if(squares[`${vNext}${column + x + 1}`]?.querySelector("img").getAttribute('value').slice(0, 5) != index) 
		...
		if(squares[`${w}${column + x}`]?.querySelector('img') != -1 && flag8 == 1) {
			glowTile(w, column + x)
			if(squares[`${wNext}${column + x + 1}`]?.querySelector('img')) {
				flag8 = 0
				if(squares[`${wNext}${column + x + 1}`]?.querySelector("img").getAttribute('value').slice(0, 5) != index) 
	...
	for (let i = row - 1; i <= row + 1; i++)
		for (let j = column - 1; j <= column + 1; j++) {
			...
			if(squares[`${z}${j}`]?.querySelector('img')){
				if (squares[`${z}${j}`]?.querySelector('img').getAttribute('value').slice(0, 5) != index) strike(z, j);
			} else glowTile(z, j);	
		}
}
		...
		for (let y = column - 2; y <= column + 2; y++, even++)
			if (x != row && y != column && even % 2 == 0) {
				...
				if(squares[`${z}${y}`]?.querySelector('img')){
					if (squares[`${z}${y}`]?.querySelector('img').getAttribute('value').slice(0, 5) != index) strike(z, y);
				} else glowTile(z, y);
			}
}
...
document.addEventListener("dragover", (event) => {
	if(event.target.classList.contains("on-top") || event.target.parentElement.classList.contains('strikeable'))
		allowNextMove = true;
